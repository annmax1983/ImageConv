...
| 功能 | 说明 |
|------|------|
| 🖼️ **4 种格式** | 转换为 PNG（无损）、JPG（高质量）、WEBP（紧凑）或 AVIF（最佳压缩，WASM 编码器真实输出） |
| 📱 **HEIC/HEIF 支持** | iPhone 照片直接转换 —— 拖拽、粘贴或右键均可，内置 WASM 解码器本地处理 |
| 📐 **调整尺寸** | 按百分比缩放或指定像素宽高，留空一边自动等比，保存与右键转换通用 |
| 📋 **复制到剪贴板** | 右键 → 复制为 PNG/JPG/WEBP/AVIF —— 直接粘贴到邮件、聊天、文档 |
| 📊 **文件大小对比** | Toast 通知显示原始大小 vs 转换后大小及节省百分比（基于真实文件体积） |
| 🎚️ **可调质量** | 通过弹窗滑块微调 JPG、WEBP、AVIF 的导出质量 |
| 🔗 **智能链接清洗** | 剥离 CDN 追踪参数（`w=`、`h=`、`format=`、`watermark=` 等）以获取原始图片 |
| 📁 **拖拽转换** | 将本地图片拖放或粘贴（Ctrl+V）到弹窗即可转换，无需右键 |
...
弹窗中的"调整尺寸"区域支持按百分比缩放（5%–200%）或指定像素宽高。留空一边可自动等比。该设置同时作用于拖拽转换和右键转换，默认保持原始尺寸。

### HEIC/HEIF（iPhone 照片）

将 `.heic` / `.heif` 文件拖入弹窗（或直接转换网页上的 HEIC 图片链接），即可转为 PNG/JPG/WEBP。解码完全在本地通过 WASM 完成，无任何上传。
...
> ℹ️ AVIF 通过内置 WASM 编码器输出（Chromium 的 Canvas 原生不支持 AVIF 编码）。超大图片（超过 2500 万像素）会自动改用 WEBP 输出并提示。
